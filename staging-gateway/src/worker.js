import { stagingPage, stagingScript } from './staging-page.js';
...
const SAFE_MESSAGES = {
  ...
  BUSINESS_SNAPSHOT_CONFIGURATION: 'The staging service is not configured correctly.',
...
function requiredEnvironment(env) {
  ...
  if (env.BUSINESS_SNAPSHOT_ENVIRONMENT !== 'NON_PRODUCTION') {
    throw new GatewayError(
      'BUSINESS_SNAPSHOT_CONFIGURATION',
      SAFE_MESSAGES.BUSINESS_SNAPSHOT_CONFIGURATION,
      503,
  ...
  if (receiver.protocol !== 'https:' || !receiver.hostname.endsWith('script.google.com')) {
    throw new GatewayError(
      'BUSINESS_SNAPSHOT_CONFIGURATION',
      SAFE_MESSAGES.BUSINESS_SNAPSHOT_CONFIGURATION,
      503,
...
function allowedOrigin(request, env) {
  ...
  const expected = configured === 'SELF' ? new URL(request.url).origin : configured;
...
function parseAndValidate(bodyText) {
  ...
  if (typeof raw.turnstileToken !== 'string' || !raw.turnstileToken.trim()) {
    throw new GatewayError(
      'BUSINESS_SNAPSHOT_VALIDATION',
  ...
  if (raw.turnstileToken.length > 2048) {
    throw new GatewayError(
      'BUSINESS_SNAPSHOT_VALIDATION',
  ...
  normalized.turnstileToken = raw.turnstileToken;
  return normalized;
}
...
async function verifyTurnstile(submission, request, env, fetcher) {
  ...
  if (
    env.BUSINESS_SNAPSHOT_TURNSTILE_ACTION &&
    result.action !== env.BUSINESS_SNAPSHOT_TURNSTILE_ACTION
  ) {
    throw new GatewayError(
      'BUSINESS_SNAPSHOT_VALIDATION',
      'Human verification failed. Please try again.',
  ...
  if (
    env.BUSINESS_SNAPSHOT_TURNSTILE_HOSTNAME &&
    result.hostname !== env.BUSINESS_SNAPSHOT_TURNSTILE_HOSTNAME
  ) {
    throw new GatewayError(
      'BUSINESS_SNAPSHOT_VALIDATION',
      'Human verification failed. Please try again.',
...
async function enforceRateLimits(submission, request, env) {
  ...
  if (!clientResult.success || !requestResult.success) {
    throw new GatewayError(
      'BUSINESS_SNAPSHOT_TEMPORARY_FAILURE',
      'The staging request limit has been reached. Please retry later.',
      429,
      'rate_limited'
    );
  }
...
async function callReceiver(
  ...
  const payload = {
    ...
    acceptedAt,
    testSecret: env.BUSINESS_SNAPSHOT_RECEIVER_SECRET,
    testClientId: `phase-d-${clientKey}`
  };
  let response;
  try {
  ...
  if (
    !result ||
    result.environment !== 'NON_PRODUCTION' ||
...
export async function handleRequest(request, env, _ctx, dependencies = {}) {
  ...
  if (request.method === 'GET' && url.pathname === '/') {
    ...
    return stagingPage(env.TURNSTILE_SITE_KEY);
  }
  if (request.method === 'GET' && url.pathname === '/staging.js') {
